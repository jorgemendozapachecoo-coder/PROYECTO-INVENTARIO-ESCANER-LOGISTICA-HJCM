...
if (isset($_POST['transfer_products'])) {
    ...
    if ($resultProducts && $resultProducts->num_rows > 0) {
        ...
        while ($product = $resultProducts->fetch_assoc()) {
            ...
            $// Obtener el último ITEM existente
...
$newItem = ($lastItemRow['last_item'] ?? 0); // Empezar desde el último ITEM

// Iterar por cada producto y transferir
while ($product = $resultProducts->fetch_assoc()) {
    $newItem++; // Incrementar ITEM progresivamente
    ...
    if ($stmtInsertInventory->execute()) {
        ...
        $mensaje .= "Error al transferir el producto con código {$product['CODIGO_PATRIMONIAL']}.<br>";
    }
}
