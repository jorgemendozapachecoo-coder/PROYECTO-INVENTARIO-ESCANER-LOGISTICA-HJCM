...
if ($resultCheckNuevos->num_rows > 0) {
    // Si el producto ya existe en PRODUCTOS_NUEVOS, mostrar formulario
    ...
    if ($resultCheckInventario->num_rows > 0) {
        // Producto existe en INVENTARIO, obtener sus datos
        ...
        if ($stmtInsert->execute()) {
            ...
            $mensaje = "Error al añadir el producto existente.";
        }
    } else {
        ...
        $mostrarFormulario = true; // Mostrar formulario con campos vacíos
    }
}
...
// Transferir productos de PRODUCTOS_NUEVOS al INVENTARIO
...
$sqlCheckDuplicate = "SELECT COUNT(*) AS total FROM INVENTARIO WHERE CODIGO_PATRIMONIAL = ?";
$stmtCheckDuplicate = $conn->prepare($sqlCheckDuplicate);
$stmtCheckDuplicate->bind_param("s", $product['CODIGO_PATRIMONIAL']);
$stmtCheckDuplicate->execute();
$resultCheckDuplicate = $stmtCheckDuplicate->get_result();
$duplicate = $resultCheckDuplicate->fetch_assoc();
...
if ($duplicate['total'] == 0) {
    ...
    // Si el producto ya existe, muestra un mensaje y omite la inserción
    $mensaje = "El producto con código patrimonial '{$product['CODIGO_PATRIMONIAL']}' ya existe en el inventario y no se duplicará.";
}
            $stmtInsertInventory->bind_param(
                "issssssssssssi",
                $newItem,
