...
if (isset($_POST['transfer_products'])) {
    ...
    if ($resultProducts && $resultProducts->num_rows > 0) {
        ...
        while ($product = $resultProducts->fetch_assoc()) {
            // Insertar cada producto en INVENTARIO
            $sqlInsertInventory = "INSERT INTO INVENTARIO (CODIGO_PATRIMONIAL, NOMBRE_AREA, CODIGO_FAMILIA_ACTUAL, DENOMINACION_BIEN, CODIGO_FAMILIA_CORRECTO, DENOMINACION, TIPO, MARCA, MODELO, SERIE, FECHA_ADQUISICION, FECHA_ACTUAL, ANTIGÜEDAD)
                                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($transferCount > 0) {
            ...
            $mensaje = "No se pudieron trasladar los productos. Por favor, intenta de nuevo.";
        }
    } else {
...
<body>
    ...
    <a href="view_inventory.php" class="btn btn-primary btn-lg">Volver al Inventario</a>
    <form method="POST" action="new_products_scan.php" class="d-inline">
        <button type="submit" name="transfer_products" class="btn btn-success btn-lg">Trasladar al Inventario Principal</button>
    </form>
