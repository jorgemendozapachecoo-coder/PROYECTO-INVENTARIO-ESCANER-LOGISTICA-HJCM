...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['codigo_patrimonial'])) {
    ...
    $sqlCheckNuevos = "SELECT * FROM PRODUCTOS_NUEVOS WHERE CODIGO_PATRIMONIAL = ?";
...
if (isset($_POST['transfer_products'])) {
    // Obtener todos los productos de PRODUCTOS_NUEVOS
    ...
    if ($resultProducts && $resultProducts->num_rows > 0) {
        ...
        // Iterar por cada producto y transferir
        while ($product = $resultProducts->fetch_assoc()) {
            // Obtener el último ITEM de la tabla INVENTARIO
            ...
            $newItem = ($lastItemRow['last_item'] ?? 0) + 1; // Incrementar ITEM
            $mensaje = "Se trasladaron {$transferCount} productos al inventario principal correctamente.";


            if ($stmtInsertInventory->execute()) {
                // Eliminar el producto de PRODUCTOS_NUEVOS después de insertarlo
...
<body>
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="text" id="nombre_area" name="nombre_area" 
            value="<?php echo htmlspecialchars($productoExistente['NOMBRE_AREA'] ?? ''); ?>" class="form-control">
    </div>
    <div class="col-md-6 mb-3">
        ...
        <input type="text" id="codigo_familia_actual" name="codigo_familia_actual" 
            value="<?php echo htmlspecialchars($productoExistente['CODIGO_FAMILIA_ACTUAL'] ?? ''); ?>" class="form-control">
    </div>
    <div class="col-md-6 mb-3">
        ...
        <input type="text" id="denominacion_bien" name="denominacion_bien" 
            value="<?php echo htmlspecialchars($productoExistente['DENOMINACION_BIEN'] ?? ''); ?>" class="form-control">
    </div>
    <div class="col-md-6 mb-3">
        ...
        <input type="text" id="codigo_familia_correcto" name="codigo_familia_correcto" 
            value="<?php echo htmlspecialchars($productoExistente['CODIGO_FAMILIA_CORRECTO'] ?? ''); ?>" class="form-control">
    </div>
    <div class="col-md-6 mb-3">
        <label for="denominacion" class="form-label">Denominación</label>
        <input type="text" id="denominacion" name="denominacion" 
            value="<?php echo htmlspecialchars($productoExistente['DENOMINACION'] ?? ''); ?>" class="form-control">
    </div>
    <div class="col-md-6 mb-3">
        <label for="tipo" class="form-label">Tipo</label>
        <input type="text" id="tipo" name="tipo" 
            value="<?php echo htmlspecialchars($productoExistente['TIPO'] ?? ''); ?>" class="form-control">
    </div>
    <div class="col-md-6 mb-3">
        <label for="marca" class="form-label">Marca <span class="text-danger">*</span></label>
        <input type="text" id="marca" name="marca" 
            value="<?php echo htmlspecialchars($productoExistente['MARCA'] ?? ''); ?>" class="form-control" required>
    </div>
    <div class="col-md-6 mb-3">
        <label for="modelo" class="form-label">Modelo <span class="text-danger">*</span></label>
        <input type="text" id="modelo" name="modelo" 
            value="<?php echo htmlspecialchars($productoExistente['MODELO'] ?? ''); ?>" class="form-control" required>
    </div>
    <div class="col-md-6 mb-3">
        <label for="serie" class="form-label">Serie</label>
        <input type="text" id="serie" name="serie" 
            value="<?php echo htmlspecialchars($productoExistente['SERIE'] ?? ''); ?>" class="form-control">
    </div>
    <div class="col-md-6 mb-3">
        <label for="fecha_adquisicion" class="form-label">Fecha de Adquisición</label>
        <input type="date" id="fecha_adquisicion" name="fecha_adquisicion" 
            value="<?php echo htmlspecialchars($productoExistente['FECHA_ADQUISICION'] ?? ''); ?>" class="form-control">
    </div>
</div>
<button type="submit" name="nuevo_producto" class="btn btn-primary w-100">Añadir Nuevo Producto</button>
                    ...
                    if ($result && $result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            $antiguedadClass = ($antiguedad > 10) ? 'antiguedad-roja' : '';
                             ...
                             echo <tr data-codigo-patrimonial="<?php echo htmlspecialchars($row['CODIGO_PATRIMONIAL'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
    <td><?php echo htmlspecialchars($row['ID'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
    <td><?php echo htmlspecialchars($row['NOMBRE_AREA'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
    <td><?php echo htmlspecialchars($row['CODIGO_FAMILIA_ACTUAL'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
    <td><?php echo htmlspecialchars($row['DENOMINACION_BIEN'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
    <td><?php echo htmlspecialchars($row['CODIGO_FAMILIA_CORRECTO'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
    <td><?php echo htmlspecialchars($row['DENOMINACION'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
    ...
    <td><?php echo htmlspecialchars($fechaAdquisicion ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
    <td><?php echo htmlspecialchars($row['CODIGO_PATRIMONIAL'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
    <td><?php echo htmlspecialchars($row['FECHA_ACTUAL'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
    <td class="<?php echo $antiguedadClass; ?>">
        <?php echo htmlspecialchars($antiguedad, ENT_QUOTES, 'UTF-8'); ?>
    </td>
    <td>
        <a href="edit_new_product.php?id=<?php echo htmlspecialchars($row['ID'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-warning btn-sm">Editar</a>
        <a href="new_products_scan.php?delete_id=<?php echo htmlspecialchars($row['ID'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" 
